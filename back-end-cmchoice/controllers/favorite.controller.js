let FavoriteModel = require('../models/favorite.model')
...
let FavoriteController = {
    createFavoriteList: async (req,res) => {
        ...
        if ( currentUserId) {
            ...
            if (user) {
                try {
                    let newList = new FavoriteModel({userId: currentUserId, ...req.body})
                    console.log("ok");
                    const listCreated = await newList.save()
                    await UserModel.findByIdAndUpdate(currentUserId, {"favorites": listCreated._id}, {new: true})
    ...
    all: async (req,res) => {
        ...
        if ( currentUserRole === "admin") {
            FavoriteModel.find().populate('products')
            .then(data => {
              res.json(data)
            })
    ...
    getFavoriteList: async (req,res) => {
        try {
            ...
            let listFind = await FavoriteModel.findById(req.params.id).populate('products')
            if (listFind) {
        ...
        if (listFind) {
            if ( listFind.userId == currentUserId ) {
                try {
                    ...
                    const updatedList = await FavoriteModel.findByIdAndUpdate(listId, req.body, {new: true})
