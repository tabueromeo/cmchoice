...
let ProductController = {
	create: async (req, res) => {
		...
		if (seller) {
			if (currentUserRole === "seller") {
				const sameProduct = await ProductModel.findOne({
					name,
				...
				if (sameProduct) {
					res
						.status(409)
						.send("You have already use this name for an other product  ");
				} else {
	...
	all: async (req, res) => {
		try {
			...
			subCategory
				...
				: (query =
						query - tag
							? (query = { ...query, tags: { $elemMatch: { tag } } })
							: (query = query));
			priceMin && priceMax
				? (query = { ...query, price: { $gte: priceMin, $lt: priceMax } })
				: (query = query);
			// query = {
	...
	updateProduct: async (req, res) => {
		...
		if (
			(productFind.sellerId == currentUserId && currentUserRole === "seller") ||
