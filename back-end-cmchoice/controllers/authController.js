...
let AuthController = {
	...
	login: async (req, res) => {
		...
		try {
			...
			if (email) {
				userFindByMail = await UserModel.findOne({ email })
					.populate("shoopingCart")
					.populate("favorites");
			} else if (phoneNumber) {
			...
			await user.populate([
				{
					path: "shoopingCart",
					populate: [{ path: "products.item" }, { path: "commands.command" }],
				},
				{ path: "favorites", populate: { path: "products.product" } },
			]);
			if (user) {
